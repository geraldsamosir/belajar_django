{% load admin_static material_admin i18n %}{% load firstof from future %}<!DOCTYPE html>
{% get_current_language as LANGUAGE_CODE %}
<html lang="{{ LANGUAGE_CODE|default:"en-us" }}">
    <head>
        <title>Admin |Gerald Blog</title>
        ...
        <header>
            <nav id="header" class="nav-top green">
                <div class="nav-wrapper">
                    ...
                    <ul class="right">
                        {% if user.is_authenticated %}
                        <li><a class="dropdown-button" href="#!" data-activates="user-menu">{{ user.get_full_name|default:user.username }}<i class="mdi-navigation-arrow-drop-down right"></i></a></li>
                        {% endif %}
                    </ul>
                </div>
                ...
                <ul id="nav-mobile" class="side-nav fixed module-menu">
                    ...
                    <li class="no-padding active">
                        <a class="collapsible-header active no-pjax" href="/admin/"><i class="mdi-action-settings-applications"></i>{% trans 'Administration' %}</a>
